"""
src/quant_research/train_v7.py

Trains GBM v7 (XGBoost): Thursday-weekly data, raw log-return target,
5-day forward return horizon.

v7 feature set (47 total: 46 numeric + sector categorical):
  v6 features (44) + 3 new earnings signals:
    ni_qoq_growth       — NI quarter-over-quarter growth (more timely than YoY)
    ni_acceleration     — YoY growth acceleration (second derivative)
    earn_ret_5d_to_20d  — Post-PEAD drift: price move from day 5 to day 20

Usage:
    python -m src.quant_research.train_v7
"""

from __future__ import annotations

import json
import pickle
from pathlib import Path

import numpy as np
import pandas as pd
import structlog

from src.quant_research.evaluate import evaluate_model, print_comparison, save_val_metrics
from src.quant_research.features_v7 import FEATURE_COLS_V7, CATEGORICAL_COLS_V7

log = structlog.get_logger()

_FEATURES_FILE = Path("data.nosync/quant/features_v7.parquet")
_ARTIFACTS_DIR = Path("data.nosync/quant/artifacts/gbm_v7")

_XGB_PARAMS = {
    "n_estimators": 600,
    "max_depth": 5,
    "learning_rate": 0.03,
    "subsample": 0.8,
    "colsample_bytree": 0.7,
    "min_child_weight": 20,
    "reg_alpha": 0.1,
    "reg_lambda": 1.0,
    "tree_method": "hist",
    "enable_categorical": True,
    "n_jobs": -1,
    "verbosity": 0,
    "random_state": 42,
}


def _load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    if not _FEATURES_FILE.exists():
        raise FileNotFoundError(
            f"Run features_v7.py first: {_FEATURES_FILE}\n"
            f"  python -m src.quant_research.features_v7"
        )

    df = pd.read_parquet(_FEATURES_FILE)
    df["date"] = pd.to_datetime(df["date"])

    df = df[df["date"].dt.dayofweek == 3].copy()
    log.info("train_v7.thursday_filter", thursday_rows=len(df))

    df = df.dropna(subset=["fwd_log_ret_5d"])
    df = df[df["fwd_log_ret_5d"].abs() <= 0.5]

    train = df[(df["split"] == "train") & (df["ticker"] != "SPY")].copy()
    val   = df[df["split"] == "val"].copy()

    log.info("train_v7.data_loaded", train_rows=len(train), val_rows=len(val))
    return train, val


def _encode_sector(
    train: pd.DataFrame,
    val: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    all_sectors = sorted(
        set(train["sector"].dropna().tolist()) | set(val["sector"].dropna().tolist())
    )
    cat_type = pd.CategoricalDtype(categories=all_sectors, ordered=False)
    train = train.copy()
    val   = val.copy()
    train["sector"] = train["sector"].astype(cat_type)
    val["sector"]   = val["sector"].astype(cat_type)
    return train, val, all_sectors


def train_gbm_v7(
    train: pd.DataFrame,
    sector_categories: list[str],
) -> None:
    """Train XGBoost v7 on raw log-return target and save artifacts."""
    import xgboost as xgb

    all_cols = FEATURE_COLS_V7 + CATEGORICAL_COLS_V7
    log.info("train_v7.gbm_v7.starting",
             rows=len(train), features=len(all_cols))

    X = train.reindex(columns=all_cols)
    y = train["fwd_log_ret_5d"].values

    model = xgb.XGBRegressor(**_XGB_PARAMS)
    model.fit(X, y)

    _ARTIFACTS_DIR.mkdir(parents=True, exist_ok=True)
    with open(_ARTIFACTS_DIR / "model.pkl", "wb") as f:
        pickle.dump(model, f)
    with open(_ARTIFACTS_DIR / "sector_categories.json", "w") as f:
        json.dump(sector_categories, f, indent=2)

    importances = sorted(
        zip(all_cols, model.feature_importances_),
        key=lambda x: x[1], reverse=True,
    )
    log.info("train_v7.gbm_v7.feature_importances", top5=[f for f, _ in importances[:5]])
    print("\nGBM v7 Feature Importances (top 20):")
    for feat, imp in importances[:20]:
        print(f"  {feat:<35} {imp:>8.4f}")

    log.info("train_v7.gbm_v7.done", artifacts=str(_ARTIFACTS_DIR))


def score_gbm_v7(df: pd.DataFrame) -> np.ndarray:
    """Score a DataFrame with the trained v7 model."""
    with open(_ARTIFACTS_DIR / "model.pkl", "rb") as f:
        model = pickle.load(f)
    with open(_ARTIFACTS_DIR / "sector_categories.json") as f:
        sector_categories = json.load(f)

    df = df.copy()
    cat_type = pd.CategoricalDtype(categories=sector_categories, ordered=False)
    df["sector"] = df["sector"].astype(cat_type)

    all_cols = FEATURE_COLS_V7 + CATEGORICAL_COLS_V7
    return model.predict(df.reindex(columns=all_cols))


def train_all_v7() -> None:
    train, val = _load_data()
    train, val, sector_categories = _encode_sector(train, val)

    train_gbm_v7(train, sector_categories)

    result = evaluate_model(
        val,
        score_gbm_v7,
        model_name="gbm_v7",
        forward_days=5,
        target_col="fwd_log_ret_5d",
        eval_weekday=3,
        top_n=20,
    )
    save_val_metrics(result)
    print_comparison([result])

    print(f"\nThursday training dates: {train['date'].nunique()} unique Thursdays")
    print(f"Thursday val dates:     {val['date'].nunique()} unique Thursdays")


if __name__ == "__main__":
    from dotenv import load_dotenv
    load_dotenv()
    train_all_v7()
