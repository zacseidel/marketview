...
class DataAccessLayer:
    ...
    def get_fundamentals(self, ticker: str) -> list[dict]:
        ...
        if ticker not in self._fundamentals_cache:
            path = self._fundamentals_dir / f"{ticker}.json"
            if path.exists():
                with open(path) as f:
                    data = json.load(f)
                # Normalize: expect a list of quarterly records
    ...
    def load_model_output(self, model: str, eval_date: str) -> list[HoldingRecord]:
        """Load a previously saved model holdings list from data/models/{eval_date}/{model}.json."""
    ...
    def save_model_output(self, holdings: list[HoldingRecord], eval_date: str, model: str) -> str:
        ...
        out_dir = Path("data/models") / eval_date
